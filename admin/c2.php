...
if ($action === 'delete' && $pieceId) {
    $result = deleteArtPiece('c2', $pieceId);
    if ($result['success']) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($action, ['create', 'edit'])) {
    ...
    if (!isset($_POST['csrf_token']) || !verifyCsrfToken($_POST['csrf_token'])) {
        ...
        if ($action === 'create') {
            $result = createArtPiece('c2', $data);
        } else {
            $result = updateArtPiece('c2', $pieceId, $data);
        }
        ...
        <form method="POST" action="" data-validate>
            ...
            <div class="form-group">
                ...
                <input
                    ...
                    class="form-control"
                    required
                    placeholder="/c2/1.php"
                    value="<?php echo $editPiece ? htmlspecialchars($editPiece['file_path']) : ''; ?>"
                >
                <small class="form-help">Relative path to the PHP file (e.g., /c2/1.php)</small>
            </div>
