...
if ($action === 'delete' && $pieceId) {
    $result = deleteArtPiece('p5', $pieceId);
    if ($result['success']) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($action, ['create', 'edit'])) {
    ...
    if (!isset($_POST['csrf_token']) || !verifyCsrfToken($_POST['csrf_token'])) {
        ...
        if ($action === 'create') {
            $result = createArtPiece('p5', $data);
        } else {
            $result = updateArtPiece('p5', $pieceId, $data);
        }
        ...
        <form method="POST" action="" data-validate>
            ...
            <div class="form-group">
                ...
                <input
                    ...
                    class="form-control"
                    required
                    placeholder="/p5/p5_1.php"
                    value="<?php echo $editPiece ? htmlspecialchars($editPiece['file_path']) : ''; ?>"
                >
                <small class="form-help">Relative path to the PHP file (e.g., /p5/p5_1.php)</small>
            </div>
