...
function generateRandomArray() {
    ...
    const length = Math.round(Math.random() * 19) + 1;
    const array = [];

    for (let i = 0; i < length; i++) {
        // Generate a random value between 0 and 20
        const randomValue = Math.round(Math.random() * 19) + 1;
...
function setupCanvas(canvasId) {
  ...
  function initializeCanvas() {
    ...
    colorChangeInterval = rects.map(() => (Math.random() * 750) + 250);
  }
